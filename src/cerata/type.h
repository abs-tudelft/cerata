...
using NodeMap = std::unordered_map<const Node *, Node *>;
using OptionalTypeMapper = std::optional<std::shared_ptr<TypeMapper>>;
...
class Type : public Named, public std::enable_shared_from_this<Type> {
 public:
  ...
  /**
   * \brief Get a mapper to another type, with the possibility to create one implicitly.
   * \param other The other type.
   * \param generate_implicit If true, attempt to generate a mapper implicitly.
   * \return Optionally a mapper.
   */
  OptionalTypeMapper GetMapper(Type *other, bool generate_implicit = true);

  /// \brief Get a mapper to another type, if it exists.
  OptionalTypeMapper GetMapper(const Type &other);
  ...
  /**
   * \brief Generate a new mapper to some other type.
   *
   * Should be checked with CanGenerateMapper first, or throws.
   *
   * \param other The other type.
   * \return A new TypeMapper from this to the other type.
   */
  ...
  /**
  * \brief Make a copy of the type, and rebind any type generic nodes.
  *
   * This is useful in case type generic nodes are on some instance graph and have to be
   * copied over to a component graph, or vice versa. In that case, the new graph has to
   * copy over these nodes and rebind the type generic nodes.
  *
  ...
  */
   ...
   * \brief Make a copy of the type, and rebind any type generic nodes.
   *
   * Rebinding is performed in order of the GetGenerics call.
   *
   * \param nodes The nodes to rebind the type to.
   * \return      The rebound type.
   */
  std::shared_ptr<Type> operator()(std::vector<Node *> nodes) const;
   ...
   * \return      The rebound type.
   */
  std::shared_ptr<Type> operator()(const std::vector<std::shared_ptr<Node>> &nodes) const;
...
class Vector : public Type {
 public:
  ...
  /// \brief Return a pointer to the width node of this vector, if specified.
  std::optional<Node *> width() const override;
...
class Field : public Named, public std::enable_shared_from_this<Field> {
 public:
  ...
  /// \brief Return if this individual field should be reversed w.r.t. parent Record.
  ...
  /// \brief Return true if a separator should be placed during name generation.
 ...
 private:
  ...
  bool reverse_;
  /// Whether this field should generate a separator for name/identifier generation in
  /// downstream tools.
  bool sep_;
};
...
/**
 * \brief Construct a new record field, and return a shared pointer to it.
 * \param type The type of the field.
 * \param reverse Whether the field is driven in reverse direction w.r.t. the parent.
 * \param sep Whether a separator for name/identifier generation should be used.
 * \return A shared pointer to the constructed field.
 */
std::shared_ptr<Field> field(const std::shared_ptr<Type> &type,
                             bool reverse = false,
                             bool sep = true);
...
class Record : public Type {
 public:
  ...
  Record &AddField(const std::shared_ptr<Field> &field,
...
/// A list of fields.
using FieldList = std::initializer_list<std::shared_ptr<Field>>;
