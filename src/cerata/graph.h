...
class Graph : public Named {
 public:
  ...
  virtual Status Add(const std::shared_ptr<Object> &object);
  /// \brief Add a list of objects to the component.
  virtual Status Add(const std::vector<std::shared_ptr<Object>> &objects);
  /// \brief Remove an object from the component
  virtual Status Remove(Object *obj);
  ...
  T *Get(const std::string &name) const {
    for (const auto &o : objects_) {
      if (o->name() == name) {
        ...
        if (result == nullptr) {
          CERATA_LOG(FATAL,
                     "Object with name " + name + " is not of type "
                         + ::cerata::ToString<T>());
        }
        return result;
      }
    ...
    CERATA_LOG(FATAL,
               "Object with name " + name + " does not exist on graph " + this->name()
                   + "\n Should be one of the following: " + ToStringAllObjects());
  }
  ...
  inline std::optional<Node *> operator()(const std::string &node_name) const {
  ...
  /// \brief Look up a parameter by the name of another parameter.
  ...
  std::string ToStringAllObjects() const;
...
class Component : public Graph {
 public:
  ...
  /// \brief Add an object to the component.
  Status Add(const std::shared_ptr<Object> &object) override;
  /// \brief Add a list of objects to the component.
  Status Add(const std::vector<std::shared_ptr<Object>> &objects) override;
  /// \brief Remove an object from the component
  Status Remove(Object *object) override;
   ...
   * \return      A pointer to the instantiated component.
   */
  Instance *Instantiate(const std::shared_ptr<Component> &comp,
...
class Instance : public Graph {
 public:
  /// Add a node to the component, returning an error if the object is a signal.
  Status Add(const std::shared_ptr<Object> &obj) override;
